...
import { DocumentService, DocumentEntry } from '../../services/document.service';
...
const IMAGE_EXTENSIONS = new Set(['png', 'jpg', 'jpeg', 'gif', 'bmp', 'heic', 'tiff', 'raw', 'webp']);
...
const FILETYPE_ICONS = new Set([
  ...
  'heic', 'html', 'java', 'jpg', 'js', 'json', 'jsx', 'key', 'md', 'mdx',
  'm4p', 'mov', 'mp3', 'mp4', 'otf', 'pdf', 'php', 'png', 'ppt', 'pptx',
  'psd', 'py', 'raw', 'rb', 'sass', 'scss', 'sh', 'sql', 'svg', 'tiff',
...
export class DocumentShellComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.url
      ...
      .subscribe(segments => {
        ...
        const extension = this.route.snapshot.queryParamMap.get('extension') ?? '';
        ...
        if (extension && extension !== 'md') {
          this.loadFileWithExtension(filePath, extension);
        } else {
          this.documentService.listDirectory(filePath)
            .pipe(takeUntilDestroyed(this.destroyRef))
            .subscribe({
              next: listing => {
                this.entries = listing.entries;
                this.title = filePath ? titleFromSegment(filePath.split('/').pop()!) : 'Documents';
                this.mode = 'directory';
              },
              error: () => {
  ...
  private loadFileWithExtension(filePath: string, extension: string): void {
    ...
    if (IMAGE_EXTENSIONS.has(ext) || VIDEO_EXTENSIONS.has(ext) || AUDIO_EXTENSIONS.has(ext) || PDF_EXTENSIONS.has(ext)) {
      this.fileType = IMAGE_EXTENSIONS.has(ext) ? 'image'
    ...
    this.documentService.getFileContent(filePath, extension)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: content => {
  ...
  private loadSidebarEntries(filePath: string): void {
    ...
    this.documentService.listDirectory(this.parentPath)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: listing => {
  ...
  private buildBreadcrumbs(filePath: string): void {
    ...
    for (const part of parts) {
      accumulated += '/' + part;
      this.breadcrumbs.push({ label: titleFromSegment(part), path: accumulated });
    }
  }
  ...
  entryLink(entry: DocumentEntry): string {
    const base = this.currentPath ? `/documents/${this.currentPath}` : '/documents';
    return `${base}/${entry.name}`;
  }

  entryQueryParams(entry: DocumentEntry): Record<string, string> | null {
    if (entry.type === 'directory' || !entry.extension || entry.extension === 'md') {
      return null;
    }
  ...
  sidebarEntryLink(entry: DocumentEntry): string {
    const base = this.parentPath ? `/documents/${this.parentPath}` : '/documents';
    return `${base}/${entry.name}`;
  }
